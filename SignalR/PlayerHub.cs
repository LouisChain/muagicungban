    ...
    public class Player : Hub
    {
        ...
        public void buy(string id)
        {
            ...
            if (Context.User.Identity.IsAuthenticated)
            {
                ...
                if (time <= 0)
                {
                    Clients.Caller.showMessage("Thời gian mua đã kết thúc!!!", 3000);
                ...
                Clients.All.updateItem(id, item.CurPrice, item.CurPrice.ToString("#,### VNĐ"), time, Context.User.Identity.Name, bid.DatePlace.ToString("dd/MM/yyyy hh:mm:ss"));

                Clients.Caller.showMessage("Xong, đơn hàng của bạn sẽ được xử lý sau 1 phút nữa", 3000);
            }
            else
            {
